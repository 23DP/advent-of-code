...
parsed = list(map(int, puzzle_input)) #list with every number, str->int 
...
def intcode_computer(input):
    ...
    while i < len(parsed):
        ...
        if operation == 99:
            ...
            rm = (instruction%1000 - instruction%100)/100 #mode for the left operand
            lm = (instruction%10000 - instruction%1000)/1000 #mode for the right operand
...
for i in range(120):
    nekst = 0
    setting = lex.nth_lex_permutation([0,1,2,3,4], i)
    for j in range(5):
        nekst = intcode_computer( [setting[j], nekst] )
    maks = maks if (maks > nekst) else nekst
